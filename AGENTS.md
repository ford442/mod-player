...
- **Compatibility:** WebGPU is the primary production visualizer. `?renderer=webgl2` selects the WebGL2 GLSL reference renderer as a real, directly-selectable session (not a no-op). If a WebGPU probe/device create fails at runtime, the app auto-falls-back to WebGL2 (or the DOM grid if WebGL2 is also unavailable) instead of hard-failing — see `applyWebGPUFallback` in `src/renderers/rendererSelection.ts`. Explicit `?renderer=html` always selects the DOM pattern grid (`PatternHTMLFallback` / `PatternSequencer`) as tracker UI. Debug: `window.__WEBGPU_PROBE__`, `window.DEBUG_RENDERER`.
...
- **Browser testing without WebGPU:** the cloud VM's Chrome has no WebGPU by default. The app now auto-falls-back to WebGL2 when the WebGPU adapter/device probe fails, so opening the dev URL with no `?renderer=` param works. Append `?renderer=webgl2` (or `?renderer=html`) explicitly if you want to force a specific fallback renderer. Audio playback requires a user gesture (click Play).
