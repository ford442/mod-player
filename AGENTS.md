...
- **Pre-existing lint state:** `npm run lint` currently exits non-zero due to 2 pre-existing errors on `main` (`utils/remoteModuleSecurity.ts` no-control-regex, `vite-env.d.ts` no-empty-object-type). This is not caused by environment setup.
