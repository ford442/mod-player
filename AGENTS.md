...
Files: `src/worklets/openmpt-processor.ts` (**source of record**) → `public/worklets/openmpt-worklet.js` (generated, tracked) + `libopenmpt-audioworklet.js` (vendor blob, tracked)
- **Never hand-edit `public/worklets/openmpt-worklet.js`.** Edit the TS source and run `npm run build:worklet-js` (esbuild IIFE; also wired as `predev`/`prebuild`). `npm run verify:worklet-js` and `npm run typecheck:worklet` are CI gates.
- The bundle compiles in `workletProtocolConstants.ts`, `mainToWorkletMessages.ts` and `utils/audioReactive.ts`, so a new message type is one TypeScript change and load time is a single `addModule()`.
...
**When editing `src/worklets/openmpt-processor.ts`:** run `npm run build:worklet-js`, commit the regenerated `public/worklets/openmpt-worklet.js`, and bump `WORKLET_VERSION` in `hooks/useWorkletLoader.ts`. Run `npm test` — `tests/workletAudioLifecycle.test.ts` enforces suspend/initLib invariants and `tests/workletProtocol.test.ts` fails if the generated file is stale.
...
- **`/src/worklets`** – TypeScript source of record for the AudioWorklet processors (`openmpt-processor.ts`, `native-bridge.ts`); compiled by `scripts/build-worklet-js.mjs` under `tsconfig.worklet.json` (ES2020, **no DOM lib**)
- **`/public/worklets`** – AudioWorklet JS processors served as static assets
  - `openmpt-worklet.js` – JS worklet processor (**generated**, tracked in git)
  - `native-bridge-processor.js` – native-engine SAB ring bridge (**generated**, tracked in git)
...
## Common Pitfalls
1. **Worklet Caching:** Browsers cache AudioWorklet files aggressively. After regenerating `openmpt-worklet.js` (from `src/worklets/openmpt-processor.ts`) or touching any worklet asset, bump `WORKLET_VERSION` and hard-refresh or disable cache in DevTools.
...
6. **Native vs JS worklet names:** Native glue is always `openmpt-native.*`. The tracked production processor is `openmpt-worklet.js`, generated by `npm run build:worklet-js` from TypeScript. Both `npm run build:emcc` and `npm run build:worklet` call `scripts/build-wasm.sh` (Emscripten) and refuse to clobber it — do not confuse `build:worklet` (native C++) with `build:worklet-js` (TS → classic script).
