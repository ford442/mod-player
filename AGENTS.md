...
- **`/utils`** – `patternExtractor.ts`, `bloomPostProcessor.ts`, `remoteMedia.ts`, `colorSchemes.ts`, `gpuPacking.ts`, `geometryConstants.ts`, `shaderVersion.ts`, plus `__debug__/packingInvariants.test.cjs`
...
- **ESLint:** ESLint is installed as a dev dependency but has no root configuration file; `npm run lint` is a no-op (`exit 0`).
