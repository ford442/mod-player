...
| P1 (hygiene) | [#442](https://github.com/ford442/mod-player/issues/442) | Ruleset on `main` (require PR + status check, block force-push) is the fix; every red run in 597–610 came from a direct `push fix` and no script prevents one. Secondary: `npm ci --dry-run --ignore-scripts` as the graph check — measured on npm 10.9.7 to catch the `65c0885` mutation in 2 s, offline, with the exact `Missing: @esbuild/… from lock file` string — plus a ~20-line `verify-lockfile.mjs` for the metadata npm skips (`integrity` / `resolved` / `lockfileVersion`, all of which pass `npm ci --dry-run` and `npm ls` silently), and a chained `npm run preflight`. Scoped to `scripts/` / `tests/` / `package.json` / workflows / docs — no overlap with #437. |
