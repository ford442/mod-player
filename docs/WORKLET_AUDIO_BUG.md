...
- [ ] **Edit `src/worklets/openmpt-processor.ts`, never `public/worklets/openmpt-worklet.js`** — the latter is generated by `scripts/build-worklet-js.mjs`. Rebuild with `npm run build:worklet-js`; `npm run verify:worklet-js` is a CI gate.
- [ ] **Bump `WORKLET_VERSION`** in `hooks/useWorkletLoader.ts` if the generated `openmpt-worklet.js` changes.
- [ ] **Extend `audio-worklet/workletProtocolConstants.ts` + `audio-worklet/mainToWorkletMessages.ts`** (not ad-hoc string literals) when adding worklet message types. The worklet compiles those modules in, so there is no classic-script mirror to keep in sync.
...
| Layer | Location | Role |
|-------|----------|------|
| Constants | `audio-worklet/workletProtocolConstants.ts` | Single source of message-type strings — compiled into the worklet bundle, not mirrored |
...
| Receive guards | `useAudioGraph.ts`, `useLibOpenMPT.ts`, `src/worklets/openmpt-processor.ts` | Validate before destructuring; reject/log malformed messages |
| Main→worklet guard | `audio-worklet/mainToWorkletMessages.ts` | Zero-dependency (no zod on the audio thread); one implementation for both sides |

`useAudioGraph` does a **single** `addModule('worklets/openmpt-worklet.js?v=N')` — the constants and the guard are bundled into that file by `scripts/build-worklet-js.mjs`.
