...
- [ ] **Edit `audio-worklet/js/openmpt-processor.ts`, never `public/worklets/openmpt-worklet.js` directly** (#413). The latter is generated by `npm run build:js-worklet` (esbuild); it carries a `// generated — do not edit` header and CI fails the drift check (`npm run build:js-worklet && git diff --exit-code`) if it's hand-edited or stale.
...
| Layer | Location | Role |
|-------|----------|------|
| Constants | `audio-worklet/workletProtocolConstants.ts` | Single source of message-type strings, imported by `protocol.ts` and the worklet's TS source (#413 — no more hand-mirrored classic-script copy) |
...
sync. #413 (2026-09) replaced that with a single TypeScript source
...
## Compiled TypeScript worklet source (#413, 2026-09)
