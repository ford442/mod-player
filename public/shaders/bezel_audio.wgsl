// bezel_audio.wgsl — hardware bezel with multi-band edge lighting (react-001)
// Extends bezel.wgsl; adds AudioReactive uniform at binding 3.
...
@group(0) @binding(3) var<uniform> audio: AudioReactive;
...
fn fs(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    // ── Audio-reactive bezel edge lighting (preserves static LED readability) ──
    if (audio.enabled > 0.5) {
      let outerR = 0.45 * outerScale;
      let edgeDist = abs(distCircle - outerR);
      let edgeBand = 1.0 - smoothstep(0.0, thickness * 2.5, edgeDist);
      let panelGlow = 1.0 - smoothstep(0.0, 0.12, abs(dRecess));
      let bassCol = vec3<f32>(0.0, 0.85, 1.0) * audio.bass;
      let midCol = vec3<f32>(0.55, 0.3, 1.0) * audio.mid * 0.65;
      let highCol = vec3<f32>(1.0, 0.5, 0.8) * audio.high * 0.45;
      let beatCol = vec3<f32>(0.95, 0.2, 0.4) * audio.beat;
      let reactive = (bassCol + midCol + highCol + beatCol) * audio.bloomDrive;
      color += reactive * edgeBand * 0.55;
      color += reactive * panelGlow * 0.12 * audio.amplitude;
      // Ambient panel wash — subtle, avoids washing out the recess
      color += bassCol * (1.0 - recessMask) * 0.06;
    }
