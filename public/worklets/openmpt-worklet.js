...
const MT = _proto ? _proto.MAIN_TO_WORKLET : {
  ...
  getOscBuffer: 'getOscBuffer', setAudioLite: 'setAudioLite', setPcmEmit: 'setPcmEmit',
};
...
class XMPlayerProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this.port.onmessage = async (e) => {
      ...
      } else if (type === MT.setPcmEmit) {
        this._pcmEmitEnabled = !!msg.enabled;
        if (!this._pcmEmitEnabled) this.pcmAccumCount = 0;
        log('PCM emit', this._pcmEmitEnabled ? 'enabled' : 'disabled');
  ...
  process(_inputs, outputs, _parameters) {
    ...
    // Position + VU + fractional row at ~60 Hz only.
    // Posting every quantum (~350 Hz) flooded the main thread (#354).
    // get_time_at_position + per-channel VU are also gated here — they were
    // the dominant extra cost on multi-channel XM at pattern starts.
