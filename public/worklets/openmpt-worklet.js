...
// Source of record: src/worklets/openmpt-processor.ts
...
(() => {
  ...
  // audio-worklet/mainToWorkletMessages.ts
  ...
  function isModulePayload(value) {
  ...
  // utils/audioReactive.ts
  var OSC_SAMPLE_COUNT = 2048;
  var AUDIO_REACTIVE_FLOATS = 16;
  var AUDIO_SAB_FLOATS = OSC_SAMPLE_COUNT + AUDIO_REACTIVE_FLOATS;
  var AUDIO_SAB_BYTES = AUDIO_SAB_FLOATS * 4;
  var AR_BASS = 0;
  var AR_MID = 1;
  var AR_HIGH = 2;
  var AR_AMPLITUDE = 3;
  ...
  // src/worklets/libopenmptWasm.ts
  var G = globalThis;

  // src/worklets/openmpt-processor.ts
  ...
  var parseMainToWorklet = parseMainToWorkletMessage;
  if (typeof G.setTimeout !== "function") {
    const _timers = /* @__PURE__ */ new Map();
    let _nextTimerId = 1;
    G.__workletTimers = _timers;
    G.setTimeout = function(fn, delayMs) {
    ...
    if (!G.__openmptWorkletLibInitPromise) {
      G.__openmptWorkletLibInitPromise = (async () => {
        ...
        if (typeof G.performance === "undefined") {
          G.performance = { now: () => currentTime * 1e3 };
        }
        if (!G.crypto || typeof G.crypto.getRandomValues !== "function") {
          installCryptoPolyfill();
        }
  ...
  var XMPlayerProcessor = class extends AudioWorkletProcessor {
    constructor(options) {
      ...
      this.port.onmessage = async (e) => {
        ...
        const type = msg.type;
        const moduleData = "moduleData" in msg ? msg.moduleData : void 0;
    ...
    async loadModule(moduleData) {
      ...
      try {
        ...
        const create = typeof lib._openmpt_module_create_from_memory2 === "function" ? lib._openmpt_module_create_from_memory2.bind(lib) : lib._openmpt_module_create_from_memory.bind(lib);
    ...
    process(_inputs, outputs, _parameters) {
      ...
      const out = outputs[0];
      const outL = out ? out[0] : void 0;
      const outR = out ? out[1] : void 0;
