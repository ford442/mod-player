...
## Today's focus
**FIX FIRST — Production load pipeline: verify + harden the parser-worker → renderer data path; deploy & close #273–#277.**
The 2026-06-17 production investigation found the live app at `test.1ink.us/xm-player/` effectively broken: the default module fetches over HTTP but **pattern data never reaches any renderer** (#274) — status stuck at `Parsing…` with no timeout/error surfacing (#276), WebGPU init failure shows a hard error overlay instead of auto-falling-back to WebGL2/HTML (#273), plus a stale `.1iss`/`utf-16` CSS artifact (#275) and deploy hygiene gaps (#277). A large fix push landed **today** (`c1e3854`): `parseInWorker` timeout/error/`messageerror` handling + main-thread fallback + progressive status in `useLibOpenMPT.ts`, `resolvePatternRendererAsync` adapter probe in `rendererSelection.ts`, `public/.htaccess` (COEP `credentialless` + CSS `charset=utf-8`), `deploy.py` asset-prune manifest + stale `.1iss` guard, and `scripts/verify-build.mjs`. **These fixes are in code but UNVERIFIED and the five issues remain OPEN.** Today's job is to verify the pipeline actually delivers pattern data on every backend, run the build+verify+deploy path, confirm production renders, and close #273–#277. Foundation work trumps new features — last week's per-instrument-palette idea already landed (v0.56, commit `5a94f07`). Save-state to `.swarm-state.md` at each iteration boundary.
...
- [ ] **PROD PIPELINE — VERIFY + CLOSE #273–#277 (today's Fix First):** big fix push landed `c1e3854` (06-19) but is UNVERIFIED and all 5 issues OPEN. Must confirm at runtime: (#274) `4-mat_madness.mod` pattern data reaches `sequencerMatrix` → renders on webgpu/webgl2/html; (#276) parser hang now resolves to `Loaded`/`Error: …` within timeout, worker 404/exception surfaces; (#273) WebGPU adapter-null auto-falls-back to WebGL2/HTML with no manual `?renderer=`; (#275) built `dist/` stylesheet ends in `.css` (no `.1iss`), served `text/css; charset=utf-8`; (#277) deploy prunes stale `assets/`, COEP `credentialless` live. Then deploy + close issues.
- [ ] **Open shader-audit follow-ups (PR #268 spinoff):** #271 octave-brightness parity for v0.46/47/48 (legacy LED shaders C-3≈C-5 look identical), #272 v0.46 overlay paging past row 64 visual regression, #269 v0.50 webglHybrid overlay decision, #270 Worklet-mode playhead prediction (~200–500 ms drift). **#271 is today's decoupled Copilot candidate (pure WGSL, zero overlap with pipeline fix).**
...
Date: 2026-06-19
Mode: Fix First
Focus: Production load pipeline — verify + harden the parser-worker → renderer data path (#274/#276), WebGPU auto-fallback (#273), and deploy/CSS hygiene (#275/#277); deploy and close the five open production issues.
Outcome: Last week's User Idea (per-instrument palettes) LANDED via merge `5a94f07` — v0.56 shader + `instrumentPalette.ts` + paletteMode toggle, typecheck/build/lint green → moved to Done. But the 2026-06-17 cursor production investigation surfaced a cracked foundation: the live app's default module never delivers pattern data to any renderer (#274), parser hangs at `Parsing…` with no timeout (#276), no WebGPU→WebGL2/HTML auto-fallback (#273), `.1iss`/`utf-16` CSS artifact (#275), deploy hygiene gaps (#277) — all five OPEN. A large fix push landed ~1 h before this run (`c1e3854`): parseInWorker error/timeout + main-thread fallback, `resolvePatternRendererAsync` adapter probe, `public/.htaccess` COEP/charset, `deploy.py` prune + verify gate, `scripts/verify-build.mjs`. Because those fixes are UNVERIFIED and the issues remain open, this is Fix First — today's deep work is verification + deploy + closing #273–#277, not a new feature. Decoupled Copilot unit = legacy LED-shader octave/sustain parity (#271, pure WGSL — zero overlap with the pipeline/renderer/deploy files Fix First touches). NOTE: chat-history tools (recent_chats/conversation_search) were unavailable this run; reconciliation done from git log + GitHub PRs/issues + `.swarm-state.md` only.
