# Weekly Plan - XASM-1 (shader blank-render cascade LANDED #346–#351 closed → but AUDIO REGRESSED A 3RD TIME #354 → FIX FIRST: audio-worklet regression lock)

## Today's focus
**FIX FIRST — Audio-worklet lifecycle regression lock: the worklet audio path has now regressed THREE consecutive cycles (#329 → #330 → #334 → #354, latest merged 2 days ago on 2026-07-29), each time re-editing the same files and the same `tests/workletAudioLifecycle.test.ts` harness that was supposed to prevent it. The #336 "audio silent-playback regression harness" was closed 2026-07-26, yet audio broke again on 2026-07-29 (MOD playback hiccups from a ~350 Hz position-postMessage flood; XM silence after module-reload from node disconnect/reconnect). That harness demonstrably does not catch the real failure modes. Today: reproduce the exact #354 failure modes as tests that FAIL on the pre-#354 code, confirm the #354 guards hold, then turn `workletAudioLifecycle.test.ts` into a behavioral harness strong enough that cycle #4 cannot ship green. No new features until the audio path is provably regression-locked.**

Why Fix First (not a feature): last week's shader-black Fix First **landed and closed** — bug cluster #346/#347/#348 (black) and #349/#350/#351 (partial) all CLOSED 2026-07-26, WGSL-duplicate-declaration follow-up merged as #353 (`29b5b6f`, 07-29), CI now runs a `visual-smoke` job (WebGL2+HTML `smoke:visual:ci` **and** WebGPU night-trio `smoke:visual`) so a black shader can't silently ship again. The whole issue tracker is now empty (0 open issues, 0 open PRs). That would ordinarily mean feature work — **except the audio foundation is visibly thrashing**: #354 (`21cb5b6`, merged 07-29) is the third worklet-audio regression in three weeks, and it landed with a documentation update to WORKLET_AUDIO_BUG.md but no *new-failure-mode* test that would have failed before the fix. A player whose audio silently dies on module reload every third week is a cracked foundation. Lock it before building anything on top.

Deep work (kimi-cli, AUDIO path only): (1) write two failing regression tests FIRST against the pre-#354 behavior — (a) worklet `position` postMessage must be throttled to ≤~60 Hz (the MOD-hiccup cause was a ~350 Hz flood), (b) on hot module-reload the AudioWorkletNode must be reused, not disconnected/reconnected, and a stale `loaded` ack with a mismatched load-token must be ignored (the XM-silence cause); (2) confirm the #354 guards in `useAudioGraph.ts`/`useLibOpenMPT.ts`/`useWorkletLoader.ts`/`openmpt-worklet.js` actually satisfy those tests; (3) audit `OpenMPTWorkletEngine.ts` + the ScriptProcessor fallback path for the same two failure modes (the #354 fix targeted the worklet path — verify the fallback can't reintroduce the flood or the silent reload); (4) leave `tests/workletAudioLifecycle.test.ts` (+ a new `tests/workletRegressionGuards.test.ts` if cleaner) as a durable harness wired into the existing CI `test` gate, documented in `docs/WORKLET_AUDIO_BUG.md`. **Boundaries:** touch only the audio path — `public/worklets/openmpt-worklet.js`, `hooks/useAudioGraph.ts`, `hooks/useLibOpenMPT.ts`, `hooks/useWorkletLoader.ts`, `audio-worklet/OpenMPTWorkletEngine.ts`/`diagnostics.ts`/`types.ts`, and `tests/worklet*.ts`. Do **NOT** touch the WebGPU render path (`hooks/useWebGPURender.ts`, `shaders/`, `utils/shaderRegistry.ts`, `components/PatternDisplay.tsx`) — that's the Copilot unit's turf this run. Invariants: no change to packing layout, to the WGSL uniform struct, or to `PatternDisplay` render call sites; bump `WORKLET_VERSION` (currently 7) if `openmpt-worklet.js` changes. Save-state to `.swarm-state.md` at each iteration boundary.
...
- **🔴 AUDIO-WORKLET REGRESSION LOCK (TODAY'S FIX-FIRST FOCUS — kimi-cli).** Worklet audio has regressed three cycles running: #329 (07-17) → #330 (07-17) → #334 (07-19) → **#354 (`21cb5b6`, merged 2026-07-29)**. #354 re-fixed MOD hiccups (~350 Hz position-postMessage flood) + XM silence on module-reload (node disconnect/reconnect + stale `loaded` ack), bumped `WORKLET_VERSION`→7, and re-touched `tests/workletAudioLifecycle.test.ts` — but with no test that FAILS on the pre-#354 code. The #336 regression harness (closed 07-26) did not catch it. Lock it: failing-first regression tests for both failure modes, confirm #354's guards satisfy them, audit the ScriptProcessor fallback for the same modes, wire a durable harness into the CI `test` gate. Audio path only. See TODAY'S FOCUS.
...
## Last run
Date: 2026-07-31
Mode: **Fix First** (worklet audio has regressed three consecutive cycles; latest #354 merged 2 days ago; the #336 harness closed 07-26 didn't catch it)
Focus: Audio-worklet lifecycle regression lock — write failing-first tests for the two #354 failure modes (position-postMessage ~350 Hz flood → MOD hiccups; node disconnect/reconnect + stale `loaded` ack → XM silence on module-reload), confirm #354's guards satisfy them, audit the ScriptProcessor fallback for the same modes, and leave a durable harness wired into the CI `test` gate so cycle #4 can't ship green. Audio path only — no render/shader/packing changes.
Outcome: Last week's shader-black Fix First LANDED and CLOSED — six render bugs #346–#351 closed 07-26, WGSL-duplicate-decl follow-up merged #353 (`29b5b6f`), CI `visual-smoke` job live. Strategic + feature issues #335–#344 / #313/#315/#317/#321/#322/#332 all closed 07-26 → **tracker empty (0 open issues, 0 open PRs)**, so no unfinished user Idea to source a focus from. That would normally be New Idea mode — **except audio regressed a third time**: #354 (`21cb5b6`, merged 07-29) re-patched MOD hiccups + XM-reload silence but shipped without a failing-first guard, and the deploy path needed a third fix (#355 stale `/xm-player/` index). A player whose audio silently dies on module reload every third week is a cracked foundation → **Fix First** overrides feature work. Decoupled Copilot unit = **per-shader visual-diff baselines** (extends the non-black CI guard to catch partial/misaligned renders like #349/#350/#351 — render/tooling files only, zero overlap with kimi's audio path). Claude Code whole-stack = build:xm-player → verify:build → `deploy.py --no-build` dry-run (re-exercises the #355 stale-index path). NOTE: chat-history tools (recent_chats/conversation_search) unavailable again this run — reconciliation done from git log + GitHub issues/PRs + `.swarm-state.md`. Local build/test NOT run (fresh container); foundation state asserted from merge history (0 open issues/PRs; #353/#354/#355 merged 07-29) + `.swarm-state.md`.
