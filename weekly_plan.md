# Weekly Plan - XASM-1 (Volume-reactive LEDs + v0.30b disc landed → Night shader trio)

## Today's focus
**USER IDEA — v0.52 / v0.53 / v0.54 "Night / Midnight / Neon Night" circular shader trio: build the three named-but-missing dark-theme circular shaders.**
Foundation is solid: last week's New Idea (volume/velocity-reactive LEDs, `patternv0.57.wgsl` / VEL-001) landed in code + merged to main; a full week of `patternv0.30b` "Note-On Disc" iteration (#286→#291) converged today (07-03) on the final "cyan holds until note-off, instant note-on, end fade" behavior; the `.env.production` base-path build footgun was fixed (#287). **0 open issues, 0 open PRs** — not Fix First. The `Ideas` list still holds two unfinished routine-generated items, so this is User Idea mode. Between them, the Night trio is the better-bounded full-day unit (self-contained new shader files + registration, all building blocks already exist) vs. the multi-day transport-UI-parity item that touches load-bearing polar hit-testing. Today's deep work: create `patternv0.52.wgsl` (Night), `patternv0.53.wgsl` (Midnight), `patternv0.54.wgsl` (Neon Night) as dark-theme circular variants building on **v0.51's playhead arc**, **v0.56's per-instrument palette**, and **Night Mode 2.0 uniform theming** (PR #207). Each is a distinct night aesthetic (deepening darkness / higher-contrast neon LEDs across the three). **No packing/struct/uniform-layout changes** — reuse the existing high-precision cell contract and three-emitter bloom profile; this is new WGSL + `shaderRegistry.ts`/`appConfig.ts`/`shaderVersion.ts` registration + `PatternDisplay.tsx`/`useWebGPURender.ts` version-chain additions. Self-verify each iteration: `npm install` then `npm run typecheck` + `npm run build` green, `npm run sync:shaders`, no WGSL parse errors, `npm run test:shader-registry` + parity tests still pass. Save-state to `.swarm-state.md` at each iteration boundary.
...
- [in progress — 2026-07-03] **v0.52 / v0.53 / v0.54 "Night / Midnight / Neon Night" shader trio** (routine-generated 2026-06-26, TODAY'S FOCUS): create the three named-but-missing dark-theme circular shaders building on v0.51 playhead arc + v0.56 per-instrument palette + Night Mode 2.0 uniform theming (PR #207). Full-day. Already tracked as a Backlog placeholder.
- [ ] **Shader-embedded transport UI parity across the v0.45–v0.56 circular family** (routine-generated 2026-06-26): v0.37 has polar hit-test play/stop/seek/volume controls; newer circular shaders (v0.45–v0.49, v0.56, v0.57, v0.30b) render without embedded UI. Extend the polar hit-test transport zones to the whole circular family so on-canvas controls work everywhere. Multi-day (touches PatternDisplay hit-testing + per-shader UI zones). Deferred again this run in favor of the better-bounded Night trio.
...
Date: 2026-07-03
Mode: User Idea
Focus: v0.52 / v0.53 / v0.54 "Night / Midnight / Neon Night" circular shader trio — the three named-but-missing dark-theme circular shaders, building on v0.51 playhead arc + v0.56 per-instrument palette + Night Mode 2.0 uniform theming. No packing/struct/uniform changes; new WGSL + registration + version-chain additions.
Outcome: Last week's New Idea (volume/velocity-reactive LEDs, `patternv0.57.wgsl`/VEL-001) LANDED in code + merged to main → moved to Done + Idea marked `[done]`. During the week Noah also did a full run of spontaneous `patternv0.30b` "Note-On Disc" shader work (#286→#291, converged today on cyan-holds-until-note-off) and fixed the `.env.production` base-path build no-op (#287) — both to Done. Tracker fully reconciled: 0 open issues, 0 open PRs (all of #252/#269/#270/#272/#280 + #278 draft closed) → NOT Fix First. Foundation solid. Ideas still held two unfinished routine-generated items, so User Idea mode: picked the Night trio (better-bounded full-day unit, all building blocks present) over the multi-day transport-UI-parity item (touches load-bearing polar hit-testing) which was deferred again. Decoupled Copilot unit this run = ChannelMeters VU-meter enhancement (peak-hold/dB/smoothing/clip in one component) — swapped in for Bug 4, which a new `utils/scrollContainer.ts` helper likely already fixed. NOTE: chat-history tools (recent_chats/conversation_search) were again unavailable this run; reconciliation done from git log + GitHub PRs/issues + `.swarm-state.md` only. Local `npm run typecheck` failed only on missing node_modules (`vite/client`, `zod`, `@webgpu/types`) in the fresh container — not a code regression.
