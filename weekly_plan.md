# Weekly Plan - XASM-1 (Night trio landed → 14 new feature issues filed → WGSL modular include system)

## Today's focus
**USER IDEA (from filed issues) — #318 Modular WGSL include system + shader build step: stop copy-paste-forking v0.5x shaders.**
Foundation is solid: last week's Night trio (`patternv0.52/0.53/0.54.wgsl` — Night/Midnight/Neon Night) LANDED and merged to main — registered in `shaderRegistry.ts` (3 blocks) + `appConfig.ts`, uniform struct matches v0.56's 132-byte layout, playhead-arc from v0.51 + per-instrument palette from v0.56 copied in; `sync:shaders`/`typecheck`/`build`/`test:shader-registry`/`test:octave-brightness`/`test:trigger-tail`/`test:duration-parity` all green per `.swarm-state.md`. Branch is even with main; **0 open PRs**. So NOT Fix First. This week Noah filed **14 feature issues (#307–#322)** — his accumulated Ideas, now formalized in the tracker. That makes this decisively User Idea mode, sourced from the issues rather than the stale `Ideas` list.

Pick rationale: **#318 is the highest-leverage well-bounded pick.** It is (a) explicitly flagged *"P1–P2 foundation for content — do before another large shader trio,"* i.e. directly motivated by the pain of the v0.50→v0.52-57 forking I just finished (there are now **9 forked `patternv0.5x.wgsl` files** duplicating `octaveBrightness`, TRIG-001 unpack, three-emitter caps, night-theme uniforms — see the multi-file octave-brightness back-port history #280); (b) squarely in Noah's active focus area (WGSL shaders); (c) full-day and file-bounded — touches `shaders/`, a new `shaders/lib/`, and the existing `scripts/sync-shaders.mjs` publish path only, NOT the load-bearing audio/hit-test/registry-contract code.

Deep work: introduce `shaders/lib/` fragments (`packing.wgsl`, `emitters.wgsl`, `polar_layout.wgsl`, `night_theme.wgsl`), evolve `scripts/sync-shaders.mjs` to resolve `#include`-style directives offline into flat WGSL in `public/shaders/` (WebGPU has no include support — production must stay flat), and refactor **≥3** circular variants (start with the fresh Night trio v0.52/0.53/0.54 since they were literally copy-pasted this week) to compose from the shared libs. Invariant: the offline-resolved output for a refactored shader must be byte-equivalent-in-behavior to today's flat file (no visual/uniform-layout change). Self-verify each iteration: `npm install` then `npm run sync:shaders`, `npm run typecheck` + `npm run build` green, no WGSL parse errors, `npm run test:shader-registry` + octave/trigger/duration parity tests still pass. Save-state to `.swarm-state.md` at each iteration boundary.
...
- [ ] **Issue-tracker feature backlog (#307–#322)** filed 2026-07-10 — see Backlog for the categorized list; routine picks today's focus from these.
...
- **GITHUB ISSUE TRACKER NOW FULL — 14 open feature issues #307–#322, all filed by Noah 2026-07-10 (0 open PRs).** These are Noah's accumulated ideas, now formalized. Categorized:
  - **P1 foundation:** #318 modular WGSL include system (TODAY'S FOCUS — kimi-cli) · #311 self-host + pin libopenmpt WASM (kill CDN SPOF) · #312 Vitest + port packing/parity scripts to real unit tests (**this run's decoupled Copilot unit**) · #307 GPU resource lifecycle / PatternDisplay memory hygiene (dispose on switch)
  - **P2 visualization/feature:** #310 shader-embedded transport UI parity (circular family) · #321 unified spectrum + multi-band reactive chassis (beyond v0.55 osc) · #320 shareable deep links (mod+shader+seek+palette as URL state) · #317 accessibility pass (high-contrast/reduced-motion/SR transport/color-blind) · #315 sample/instrument browser + waveform · #314 offline-capable PWA
  - **P2–P3 platform/feature:** #316 native C++ worklet promotion (SAB ring-buffer default + CI artifact) · #319 MIDI/gamepad/HID control surface · #313 WAV/stem export + MediaRecorder capture · #322 lightweight pattern editor mode
...
- **THIS RUN'S decoupled Copilot candidate → #312 Vitest test harness + port packing/parity scripts to real unit tests.** Touches `package.json`, `vitest.config.ts` (new), and `utils/__debug__/*.test.cjs` → `*.test.ts`. **Zero file overlap** with the `shaders/` + `shaders/lib/` + `scripts/sync-shaders.mjs` files the #318 kimi-cli run touches; packing bit-layout is unchanged by #318, so the ported tests stay valid. Labeled "good first issue" — ideal Copilot scope. (Prior candidate — ChannelMeters VU peak-hold/dB/clip — remains a clean decoupled fallback if #312 is deferred.)

## Done
- [x] 2026-07-10 — NIGHT TRIO v0.52/0.53/0.54 (last week's User Idea focus): `patternv0.52.wgsl` (Night), `patternv0.53.wgsl` (Midnight), `patternv0.54.wgsl` (Neon Night) — dark-theme circular variants, uniform struct matching v0.56 (132-byte, paletteMode slot 32, instrumentPalette @binding(7)), v0.51 playhead-arc overlay copied verbatim (ARC-001 + bottom-12% UI-band discard), v0.56 per-instrument palette sampling path, three-emitter/DURA/ACES unchanged. Registered in shaderRegistry.ts + appConfig.ts. `sync:shaders`/`typecheck`/`build`/`test:shader-registry`/`test:octave-brightness`/`test:trigger-tail`/`test:duration-parity` all green (`.swarm-state.md`). Merged to main (branch even with main, 0 open PRs). Runtime browser verify pending → Backlog.
...
## Last run
Date: 2026-07-10
Mode: User Idea (sourced from newly-filed issues)
Focus: #318 Modular WGSL include system + shader build step — introduce `shaders/lib/` fragments, evolve `scripts/sync-shaders.mjs` to resolve `#include` offline into flat `public/shaders/` WGSL, refactor ≥3 circular variants (start with the just-forked Night trio) to compose from shared libs. No visual/uniform-layout change; behaviour-equivalent flat output.
Outcome: Last week's Night trio (v0.52/0.53/0.54) LANDED + merged to main → Done + Idea marked `[done]`; 9 forked `patternv0.5x.wgsl` files now duplicate shared logic, which is exactly what #318 addresses. Pipeline-hygiene PR #294 merged. BIG CHANGE: tracker went from empty (last run) to **14 open feature issues #307–#322**, all filed by Noah 2026-07-10 — his accumulated ideas formalized. Reconciled into Backlog by priority band. NOT Fix First (branch even with main, 0 open PRs, all last-week tests green per `.swarm-state.md`). User Idea mode from the issues: picked #318 (highest-leverage, well-bounded, explicitly "do before another large shader trio," in the active shader focus area) over #311 self-host WASM / #307 GPU lifecycle / #312 Vitest. Transport-UI-parity Idea reconciled → now issue #310 (deferred, multi-day). Decoupled Copilot unit = #312 Vitest harness (good-first-issue, zero file overlap with #318's shader/sync work). NOTE: chat-history tools (recent_chats/conversation_search) were unavailable again this run; reconciliation done from git log + GitHub issues/PRs + `.swarm-state.md`. node_modules absent in fresh container so no local build/typecheck was run — foundation "green" is asserted from `.swarm-state.md` + merge state, and runtime browser verification of the Night trio remains standing debt in Backlog.
