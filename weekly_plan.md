# Weekly Plan - XASM-1 (#318 include system + Vitest landed → production audio cascade → FIX FIRST: silent-playback stabilization)

## Today's focus
**FIX FIRST — Audio playback stabilization: finalize + verify the silent-playback fix (draft PR #330), confirm #329 + #330 together fully cure the "Playing but no sound" regression across every load path, and land a regression guard so this class of bug can't silently return.**

Why Fix First (not a new feature): last week's #318 (WGSL include system + Night-trio migration) and #312 (Vitest, 50 tests) both **landed and merged** — that work is solid and green. But this week the **core function of a music player broke in production**. In rapid succession: `.env.production` base-path emitted 404 asset URLs at site root (#326, merged), `deploy.py` could upload a wrong-profile `dist/` (#327, merged), and — critically — **XM playback went silent after switching modules** with a runaway `ended → seek 0` loop because every `play()` re-initialised libopenmpt in the *shared* `AudioWorkletGlobalScope`, corrupting the WASM heap (#329, merged 07-17). A **second** silent-playback bug remains **unmerged**: `stopMusic(false)` suspended the `AudioContext` on every module reload, and `resume()` after `suspend()` needs a *fresh* user gesture — so auto-play from the file picker / storage playlist / share-URL fired after the click gesture had expired and played silently. That fix is deployed to `test.1ink.us` and manually verified **but sits in draft PR #330, not merged to main**. The foundation is cracked and only half-patched; nothing new gets built on it until playback is provably solid on every path.

Deep work (kimi-cli, audio domain only): (1) get PR #330's AudioContext-keep-alive fix merged into `main` — review it, reconcile with #329 which is already on main, resolve any conflict, confirm typecheck/lint/`npm test` (50) + `build:xm-player:verify` all green; (2) trace and enumerate **every** playback entry path — default-module auto-load, file-picker load, storage-playlist item, share/deep-link URL, MOD→XM and XM→MOD switch, stop→reload same module — and prove each reaches audible output (worklet path, not just ScriptProcessor fallback), no `ended/seek` loop, no suspended context; (3) add an automated regression guard (Playwright/headless or a unit-level worklet-lifecycle assertion) that fails if `play()` re-inits the shared libopenmpt scope or if a module reload leaves the context suspended — the two exact root causes of #329/#330; (4) confirm the worklet cache-bust (`WORKLET_VERSION`/service-worker `mod-player-v4`) is coherent so users don't get a stale broken worklet. **Boundaries:** touch only the audio path — `public/worklets/openmpt-processor.js`, `openmpt-worklet.js`, `audio-worklet/`, `hooks/useAudioGraph.ts`, `hooks/useLibOpenMPT.ts`, `hooks/useWorkletLoader.ts`, and the `play()`/auto-play call sites. Do **not** touch shaders, `shaders/lib/`, the WebGPU render pipeline, or the pattern-packing/registry code (all green from #318). Invariant: no change to shader uniforms, packing layout, or visual output. Save-state to `.swarm-state.md` at each iteration boundary.
...
- **🔴 PRODUCTION AUDIO STABILITY (TODAY'S FIX-FIRST FOCUS — kimi-cli).** This week's regression cascade — track to fully-resolved:
  - **#329 (MERGED 07-17)** — worklet singleton: init libopenmpt once per `AudioWorkletGlobalScope`, reuse the node across module reload, keep node alive across `stopMusic(false)`. Cured the silent-XM-after-switch + runaway ended/seek loop.
  - **#330 (OPEN DRAFT — must merge)** — AudioContext keep-alive: don't `suspend()` on normal stop/reload (resume-after-suspend needs a fresh gesture → silent auto-play from picker/playlist/share-URL); guard duplicate `play()`; tighten native-engine probe to require `openmpt-native.wasm`; cache-bust worklet `?v=6` + SW `mod-player-v4`. Deployed + manually verified on test.1ink.us, **not yet merged to main.**
  - **Standing debt:** add a regression guard for both root causes (no re-init of shared libopenmpt scope on `play()`; no suspended context after reload). Worklet-vs-ScriptProcessor path must be confirmed audible on every entry path.
...
## Last run
Date: 2026-07-17
Mode: **Fix First** (production audio playback broke this week; fix half-landed)
Focus: Audio playback stabilization — finalize + merge silent-playback fix (draft PR #330), confirm #329 + #330 together cure "Playing but no sound" across every load path (default/auto-load, file picker, storage playlist, share URL, MOD↔XM switch, stop→reload), and add a regression guard for both root causes (no re-init of the shared libopenmpt worklet scope on `play()`; no suspended AudioContext after reload). Audio path only — no shader/registry/packing changes.
Outcome: Last week's #318 (WGSL include system + Night-trio migration) and #312 (Vitest, 50 tests) both LANDED + merged → Done, Ideas marked `[done]`. But the week's dominant event was a **production-audio regression cascade**: base-path 404s (#326, merged), deploy building the wrong profile (#327, merged), and silent XM-after-module-switch with a runaway `ended→seek 0` loop from worklet WASM-heap corruption (#329, merged). A **second** silent-playback bug (AudioContext `suspend()` on reload → resume needs a fresh gesture → silent auto-play) is fixed + deployed but sits in **unmerged draft PR #330**. Foundation cracked and only half-patched → **Fix First** (overrides the User-Idea cascade; no new feature until playback is provably solid). Decoupled Copilot unit switched to a **visual-only accessibility slice of #317** (reduced-motion + color-blind palettes + high-contrast) — zero overlap with the audio files kimi-cli touches; deliberately excludes #317's ARIA-transport piece (audio-adjacent). Claude Code whole-stack task = re-exercise the deploy pipeline (build → build:xm-player:verify → deploy.py dry-run), since base-path/deploy broke twice this week. NOTE: chat-history tools (recent_chats/conversation_search) unavailable again this run; reconciliation done from git log + GitHub issues/PRs + `.swarm-state.md`. `node_modules` absent in the fresh container, so no local build/test was run — foundation state asserted from merge history + PR bodies (#329/#330 both report typecheck/lint/`npm test`/`build:xm-player:verify` green) + `.swarm-state.md`.
