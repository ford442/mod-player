# Weekly Plan - XASM-1 (audio Fix First LANDED #330/#334 → Noah's live-QA filed 6 shader-blank bugs #346–#351 → FIX FIRST: shader blank-render cascade)

## Today's focus
**FIX FIRST — Shader blank-render cascade: fix the family of production shaders that render solid black in the live browser, confirmed by Noah's own 2026-07-22 QA. Primary cluster #346 (Night trio v0.52/0.53/0.54 black), #347 (v0.56 Instrument Palette black), #348 (v0.55 Oscilloscope intermittently black) — they share a texture-binding / bind-group-rebuild-on-shader-switch / theme-contrast root cause. Prove each renders visible LEDs during playback and add a visual-smoke guard so a black shader can't ship again.**

Why Fix First (not a new feature): last week's audio Fix First **landed and is solid** — PR #330 (AudioContext keep-alive) merged (`539f0a3`), the #334 fractional-`noteAge` follow-up merged (`5fbd056`), #333 offline WAV export merged; **0 open PRs**, audio foundation green. But a new, worse crack surfaced: on **2026-07-22 Noah (ford442) filed six shader-rendering bug reports #346–#351 from live QA on `test.1ink.us`** — multiple production shaders render **blank/black or partial**. The visualization engine is the product; a shader family outputting black is a cracked foundation. The plan carried these exact shaders as "landed + merged, runtime verify pending" for weeks — the verification finally happened and they're broken. No new visual feature gets built until the picker's shaders provably render.

Deep work (kimi-cli, WebGPU render domain only): (1) reproduce #346/#347/#348 headlessly via `npm run smoke:visual` / `?renderer=webgl2` + `window.currentPatternRenderer.readPixels()` — assert non-black output during playback; (2) root-cause the black cluster — likely (a) instrument-palette 1-D texture (`@binding(7)`) and osc texture (`@binding(6)`) not created/bound before first draw on shader switch, and/or `paletteMode` uniform left 0 with no fallback path (#347), (b) osc texture lifecycle not rebuilding bind groups on shader-switch / play-stop (#348), (c) `shaders/lib/night_theme.wgsl` `THEME_BG`/`THEME_LED_OFF` ~0.025 plus unset night uniforms (`themeBlend`/`nightPreset`/`innerRadius`/`outerRadius`) collapsing all LEDs to background (#346); (3) fix so bind groups are rebuilt whenever the active shader's texture set changes and every picker shader has a non-black fallback when its optional texture/uniform is absent; (4) add a CI visual-smoke assertion (extend `scripts/visual-smoke.mjs` / `smoke:visual:ci`) that fails if v0.52–v0.57 render an all-black frame during playback. **Boundaries:** touch only the WebGPU render path — `hooks/useWebGPURender.ts` (texture lifecycle + bind-group rebuild), `shaders/lib/night_theme.wgsl` + `circular_night_body.wgsl`, `utils/shaderRegistry.ts` (uniform/texture wiring only), the affected `shaders/patternv0.5{2,3,4,5,6}.wgsl` (+ `public/shaders/` regen via `sync:shaders`), and `scripts/visual-smoke.mjs`. Do **not** touch the audio path (`public/worklets/`, `audio-worklet/`, `hooks/useLibOpenMPT.ts`/`useAudioGraph.ts`/`useWorkletLoader.ts` — just stabilized). Do **not** touch the video-family shaders v0.23/v0.24 (#349/#350) or the WebGL2 overlay 32/64 alignment (#351) — those are a separate second cluster. Invariant: no change to packing layout or `play()` call sites. Save-state to `.swarm-state.md` at each iteration boundary.
...
- [ ] **Issue-tracker feature backlog** — 14 filed 2026-07-10; **9 now closed** (#307/#310/#311/#312/#314/#316/#318/#319/#320), **5 open**: #313 WAV/stem export · #315 sample/instrument browser · #317 accessibility pass · #321 unified spectrum reactive chassis · #322 pattern editor mode. Routine picks the next User-Idea focus from these once the audio foundation is re-stabilised.
...
- **🔴 SHADER BLANK-RENDER CASCADE (TODAY'S FIX-FIRST FOCUS — kimi-cli).** Noah's 2026-07-22 live QA on `test.1ink.us` filed six render bug reports. Primary black cluster kimi-cli owns this run:
  - **#346** — Night trio v0.52/0.53/0.54 render solid black (WebGPU inits OK). Suspect `shaders/lib/night_theme.wgsl` `THEME_BG`/`THEME_LED_OFF` ~0.025 + unset night uniforms (`themeBlend`/`nightPreset`/`innerRadius`/`outerRadius`).
  - **#347** — v0.56 Instrument Palette renders black. Suspect palette 1-D texture `@binding(7)` not bound before draw and/or `paletteMode` uniform left 0 with no fallback.
  - **#348** — v0.55 Oscilloscope intermittently black after shader-switch / play-stop. Suspect osc texture `@binding(6)` lifecycle + stale bind groups.
  - **Standing debt (now landing this run):** a CI visual-smoke assertion that fails if any picker shader (v0.52–v0.57) renders an all-black frame during playback — the missing guard that let these ship "landed + merged, verify pending" and stay broken for weeks.
- **🟠 SECOND RENDER CLUSTER (deferred — NOT this run's kimi focus; do not touch alongside #346–#348).** #349 v0.24 Tunnel partial band, #350 v0.23 Clouds scanline misaligned (both video-family `chassis_video.wgsl` full-frame coverage), #351 WebGL2 hybrid overlay misaligned vs 32/64 step modes (`src/renderers/webgl2/WebGL2PatternRenderer.ts`). Overlaps kimi's shared files (`shaderRegistry`/`useWebGPURender`) so sequenced after the black cluster, not in parallel.
- **✅ PRODUCTION AUDIO STABILITY — RESOLVED (last week's Fix First, now Done).** #329 (worklet singleton, merged 07-17) + #330 (AudioContext keep-alive, merged `539f0a3`) + #334 (fractional `noteAge` sync follow-up, merged `5fbd056`) together cured the silent-playback cascade; #333 offline WAV export also merged. The **regression-guard debt** (assert `play()` never re-inits the shared libopenmpt scope; assert reload never leaves the context suspended) is now formalized as open issue **#336** and is THIS RUN'S decoupled Copilot unit (tests + audio hooks only — zero overlap with kimi's shader-render work).
...
## Last run
Date: 2026-07-24
Mode: **Fix First** (Noah's live QA confirmed a family of production shaders renders solid black)
Focus: Shader blank-render cascade — fix the black cluster #346 (Night trio v0.52/0.53/0.54), #347 (v0.56 Instrument Palette), #348 (v0.55 Oscilloscope), all sharing a texture-binding / bind-group-rebuild-on-shader-switch / night-theme-contrast root cause; prove each renders visible LEDs during playback and land a CI visual-smoke guard that fails on an all-black frame. WebGPU render path only — no audio/packing/`play()` changes. Second render cluster (#349/#350 video family, #351 webgl2 overlay) deferred to avoid file collision with the black cluster.
Outcome: Last week's audio Fix First LANDED cleanly — #330 (AudioContext keep-alive, `539f0a3`) + #334 (fractional noteAge, `5fbd056`) + #333 (offline WAV export) all merged; **0 open PRs**, audio foundation green → moved to Done, regression-guard debt reformulated as open issue #336. But the dominant new signal is Noah's own **2026-07-22 live-QA bug reports #346–#351**: multiple picker shaders render blank/black/partial on `test.1ink.us`. The visualization engine is the product; a shader family outputting black is a cracked foundation → **Fix First** (overrides the feature backlog — #335–#344 strategic issues wait until the picker provably renders). Decoupled Copilot unit = **#336 audio silent-playback regression harness** (tests + audio hooks only — zero overlap with kimi's WebGPU-render files); it's P0 standing debt from last week's audio work and safely outside the render path kimi touches. Claude Code whole-stack task = build → **visual-smoke** (the exact CI gate that would have caught #346–#348) → `deploy.py` dry-run. NOTE: chat-history tools (recent_chats/conversation_search) unavailable again this run — reconciliation done from git log + GitHub issues/PRs + `.swarm-state.md`. `node_modules` absent in the fresh container, so no local build/test was run — foundation state asserted from merge history (0 open PRs; #330/#333/#334 merged) + the six Noah-filed render issues + `.swarm-state.md`.
