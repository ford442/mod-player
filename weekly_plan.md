# Weekly Plan - XASM-1 (audio regression LOCK landed #362 → but XM pattern-boundary stutter regressed a 4TH cycle #375/#376 + slider bug #372 → app went PUBLIC v0.30b → FIX FIRST: real-browser audio product-flow smoke #380)

## Today's focus
**FIX FIRST — Real-browser audio product-flow smoke (#380, P0): the unit-level regression lock landed last week (#362 behavioral guards + #364 typed worklet protocol), yet within days the audio path regressed a FOURTH time — XM pattern-boundary stutter from per-quantum worklet CPU overshoot (#375 + #376, WORKLET_VERSION 7→9, both merged 08-02) — plus a master-output slider bug (#372, volume/pan not reaching the destination chain). None of these were caught by the Vitest guards, which assert pure helpers and source-string invariants, not a live AudioWorkletNode + libopenmpt session. As #380 states plainly: "CI can currently ship green while speakers are silent." Today: implement #380 — a headless Playwright product-flow smoke that runs against `npm run preview` with `?engine=js`, auto-loads a MOD, clicks Play, asserts position/order/row counters advance over ≥1–2s, loads an XM mid-playback and asserts counters keep advancing with no worklet `error`/`ended→seek 0` loop, stops→plays again, and fails on any worklet/protocol console error. Consume the `?audioDiag=1` / `window.__AUDIO_DIAG__` timing hook #375 already added to also fail on pattern-boundary quantum overruns. Wire into CI next to `visual-smoke`. This is the durable net that finally catches the silent/hiccup/stutter modes four unit-test cycles have missed.**

Why Fix First (not a feature or a refactor): last week's audio-lifecycle lock **did land** — Copilot #362 ("lock #354 worklet lifecycle regressions with behavioral guards", merged 07-31) + #364 (typed validated worklet message protocol) + the decoupled Copilot visual unit #363 (structural coverage assertions for visual smoke). But the lock is unit-level and **provably insufficient**: the app then regressed audio a 4th time (#375/#376 XM pattern-boundary stutter) and shipped a broken master-gain/pan chain (#372), all green in CI. Meanwhile Noah made a decisive product pivot — v0.30b locked as the **public default** with a minimal public surface (#366–#369, #373; `VITE_PUBLIC_MODE=1`, version→0.1.0, SW cache `mod-player-v5`, 3D/media/export/MIDI/edit gated off). A player that just went **public** but whose CI can ship silent audio is a cracked foundation under a freshly public roof. #380 (P0, filed 08-05) is the exact fix and takes priority over the six open refactor/hygiene issues.

Deep work (kimi-cli, TEST/CI/TOOLING path only): implement #380 end-to-end. (1) New `scripts/audio-smoke.mjs` (or Playwright spec under `tests/e2e/`) driving `npm run preview` headless Chromium with `?renderer=webgl2&engine=js` so no native artifacts are needed; (2) auto-load default module → Play → assert counters advance → load a 2nd format (`.xm` fixture) mid-play → assert advance + no error → Stop→Play→assert advance; (3) subscribe to `page.on('console')` and the exposed `window.__AUDIO_DIAG__` (from #375's `?audioDiag=1`) — fail on worklet/protocol errors AND on any pattern-boundary quantum that exceeds budget; (4) add `npm run smoke:audio` / `smoke:audio:ci` scripts and a CI job beside `visual-smoke`; (5) update `docs/WORKLET_AUDIO_BUG.md` to mark which checks are now automated vs still manual. **Boundaries:** touch ONLY `scripts/`, `tests/` (new e2e spec + any Vitest helper), `.github/workflows/`, `package.json` scripts, `docs/WORKLET_AUDIO_BUG.md`, and — if strictly needed — a **read-only** test hook that surfaces counters/diag on `window` (do not change audio behavior). Do **NOT** edit `public/worklets/openmpt-worklet.js`, `hooks/useAudioGraph.ts`/`useLibOpenMPT.ts`/`useWorkletLoader.ts`, `audio-worklet/*` engine logic, or ANY shader/render file (`shaders/`, `utils/shaderRegistry.ts`, `components/PatternDisplay.tsx`, `hooks/useWebGPURender.ts`) — the render/UI path is the Copilot unit's turf this run. Invariants: no runtime behavior change, no new production dependency (Playwright is already in devDependencies), no `WORKLET_VERSION` bump (this run adds no worklet edits). Save-state to `.swarm-state.md` at each iteration boundary.

## Ideas
<!-- User-written ideas Noah accumulates during the week. Routine prioritizes these. -->
- [done — 2026-07-03] **Volume/velocity-reactive LED brightness + bloom** (routine-generated 2026-06-26): LANDED as `patternv0.57.wgsl` (VEL-001) — bottom emitter repurposed to a cyan velocity/volume indicator for pitched notes, top/mid emitter intensity scaled by `normalizedCellVolume()`, organic sin-pulse on sustain tails. No packing changes (volume decoded from existing packed bytes). Registered in shaderRegistry/appConfig/shaderVersion + PatternDisplay/useWebGPURender chains; typecheck + build + all parity tests green (see `.swarm-state.md` VEL-001). Runtime browser verify still pending → Backlog.
- [done — 2026-07-10] **v0.52 / v0.53 / v0.54 "Night / Midnight / Neon Night" shader trio** (routine-generated 2026-06-26): LANDED + merged to main. Three dark-theme circular variants building on v0.51 playhead arc + v0.56 per-instrument palette + Night Mode 2.0 uniform theming. Files `shaders/patternv0.5{2,3,4}.wgsl` (+ public copies), registered in `shaderRegistry.ts`/`appConfig.ts`, uniform struct matches v0.56 132-byte layout. sync/typecheck/build + all parity tests green (`.swarm-state.md`). Runtime browser verify still pending → Backlog. NOTE: the copy-paste forking this required is exactly what motivated issue #318 (today's focus).
- [tracked as issue #310] **Shader-embedded transport UI parity across the circular family** (routine-generated 2026-06-26): now formalized as GitHub issue #310 (v0.45–v0.57). Still deferred — multi-day, load-bearing polar hit-testing, and #310 itself notes it should follow the SHADER_REGISTRY migration + playhead foundation. No longer tracked as a loose Idea here; lives in the issue tracker.

<!-- Noah's live ideas now live primarily in the GitHub issue tracker as of 2026-07-10 (#307–#322). Routine sources User-Idea focus from there. -->
- [in progress — 2026-08-07] **#380 real-browser audio product-flow smoke** (P0, filed 08-05) — today's Fix First. The tracker is NO LONGER empty: 7 open issues filed 08-01/08-05 (#370/#371/#380/#381/#382/#383/#384), all hygiene/refactor/DX. Once #380 lands, the next non-Fix-First run sources a User-Idea focus from these (the three file-split refactors #370/#381/#382, or #384 native-engine parity).
- [done — 2026-07-17] **#318 Modular WGSL include system + shader build step** (last week's User-Idea focus): LANDED + merged. `scripts/sync-shaders.mjs` now resolves `//#include` offline into flat `public/shaders/`; `shaders/lib/` fragment library created (`notes`, `pitch`, `dura`, `palette`, `sdf`, `tonemap`, `color_preserve`, `top_emitter`, `lens_cap` + `theme_night_52/53/54`); Night trio v0.52/0.53/0.54 fully migrated (byte-identical / comment-only diffs), v0.51 partially migrated as a cross-family proof point. `tests/shaderIncludes.test.ts` (5/5) added; tsc build fixed to exclude Node-only tests (#324). All parity/registry tests green (`.swarm-state.md` #318 log).
- [done — 2026-07-17] **Vitest unit-test harness (#312)** (last week's decoupled Copilot unit): LANDED — `npm test` now runs 50 Vitest tests; packing/parity scripts ported. Confirmed green in #324/#325/#329 CI notes.
- [exhausted — 2026-07-31] **Issue-tracker feature backlog** — every feature issue is now CLOSED. The 14 filed 2026-07-10 and the strategic set #335–#344 filed 2026-07-21 are all closed as of 2026-07-26; the six render bugs #346–#351 closed the same day. **Tracker is empty (0 open issues, 0 open PRs).** No unfinished user-written Idea remains to source a User-Idea focus from. Next non-Fix-First run will be New Idea mode (or Noah adds fresh Ideas here / files new issues). Strong candidates when the audio foundation is locked: instrument/sample inspector (was #342), unified spectrum reactive chassis (was #321), accessibility visuals (was #341/#332).
...
- **🔴 REAL-BROWSER AUDIO PRODUCT-FLOW SMOKE #380 (TODAY'S FIX-FIRST FOCUS — kimi-cli).** Audio regressed a 4th cycle (#375/#376 XM pattern-boundary stutter, 08-02) plus a master-output slider bug (#372) — all green under the #362 unit guards. Unit-level guards assert helpers/source-strings, not a live worklet session, so "CI can ship green while speakers are silent." Implement #380: headless Playwright product-flow smoke (preview + `?engine=js`), play→MOD↔XM→stop→play with advancing counters, fail on worklet/protocol console errors and on `window.__AUDIO_DIAG__` pattern-boundary overruns, CI job beside `visual-smoke`. Test/CI/tooling files only. See TODAY'S FOCUS.
- **🟢 AUDIO-WORKLET LIFECYCLE LOCK (#354 modes) — LANDED (#362/#364, 07-31).** Behavioral guards for the position-flood / node-reuse / stale-ack modes are in `tests/workletRegressionGuards.test.ts` + `tests/workletAudioLifecycle.test.ts`; typed worklet protocol boundary (#364) validates messages both directions. Keep these — #380 compounds with them, it does not replace them.
- **🟡 TRACKER: 7 OPEN ISSUES (all hygiene/refactor/DX; filed 08-01/08-05, mostly by cursor bot).** **P0:** #380 (Playwright audio smoke — TODAY) · #381 (split `hooks/useAudioGraph.ts` <700 lines). **P1:** #382 (thin `PatternDisplay.tsx` — finish renderer-host extraction) · #383 (quarantine tracked `a.out.*` Emscripten leftovers + agent scratch `.swarm-state.md`/`weekly_plan.md` from product tree; optional ESLint max-warnings ratchet 100→~33). **Refactor:** #370 (split `useLibOpenMPT.ts`) · #371 (split `App.tsx`). **P2:** #384 (native-engine sample-accurate clock + A/V parity gate before prefer-native). The three file-split refactors (#370/#381/#382) should follow #380 landing so the audio smoke can catch regressions the splits might introduce.
- **🟡 v0.30b IS NOW THE PUBLIC FACE (verify + polish debt).** v0.30b locked as public default (#369, 08-01). Runtime browser verification of the public build is now load-bearing: confirm `?` public surface hides 3D/media/export/edit, minimal Controls render, v0.30b renders correctly at `/xm-player/` base, SW `mod-player-v5` cache-busts cleanly. This run's Claude Code hygiene task re-exercises the public build+deploy path.
- **🟡 DEPLOY / LOCKFILE FRAGILITY (watch — 4th deploy-area churn).** Prior deploy fixes #326/#327/#355; this week package-lock.json was deleted then restored (#374/#378) and `.cursor/environment.json` added for Cloud `npm ci`. The Claude Code whole-stack hygiene task re-exercises `build:xm-player` → `verify:build` → `deploy.py` dry-run AND `npm ci` against the restored lockfile.
- **✅ SHADER BLANK-RENDER CASCADE — LANDED + CLOSED (last week's Fix First, now Done).** All six of Noah's 2026-07-22 render bugs are CLOSED (07-26): black cluster #346 (Night trio v0.52/0.53/0.54), #347 (v0.56 Instrument Palette), #348 (v0.55 Oscilloscope) and partial cluster #349 (v0.24 Tunnel), #350 (v0.23 Clouds), #351 (WebGL2 overlay 32/64). Root causes fixed per `.swarm-state.md` (osc/palette texture lifecycle, empty-palette hue ramp, night-theme floor levels); WGSL duplicate-declaration follow-up merged as **#353** (`29b5b6f`, 07-29). CI now has a `visual-smoke` job running `smoke:visual:ci` (WebGL2+HTML) **and** WebGPU night-trio `smoke:visual`. **Standing gap → this run's decoupled Copilot unit:** the CI guard only asserts *non-black* — it would NOT have caught the #349/#350/#351 *partial/misaligned* renders. Copilot adds per-shader pixel-diff baselines so partial regressions are caught too (render/tooling files only — zero overlap with kimi's audio path).
- **🟡 DEPLOY STALE-INDEX (fixed 07-29, watch).** #355 (`f77dd6a`, merged 07-29) — a stale `/xm-player/` directory index served an old bundle and broke XM audio on `test.1ink.us`. `deploy.py` now detects it. This is the 3rd deploy/base-path fix (#326/#327/#355) → the Claude Code whole-stack hygiene task re-exercises build:xm-player → verify:build → deploy dry-run.
- **DEPLOY/BASE-PATH FRAGILITY (fixed this week — keep an eye on):** #326 default prod build back to root base (`/`), #327 `deploy.py` runs `build:xm-player:verify` by default (only `--no-build` skips), `scripts/verify-build.mjs` rejects root-base index when `EXPECTED_BASE=/xm-player/`. The Claude Code whole-stack hygiene task should re-exercise this since it broke twice.
- **GITHUB ISSUE TRACKER: 5 open (0 non-draft PRs; #330 open draft).** Of the 14 filed 2026-07-10, **9 closed** (#307/#310/#311/#312/#314/#316/#318/#319/#320). Remaining open feature issues:
  - **P2 visualization/feature:** #321 unified spectrum + multi-band reactive chassis (beyond v0.55 osc) · #317 accessibility pass (high-contrast/reduced-motion/SR transport/color-blind — **this run's decoupled Copilot unit sources a visual-only slice of this**) · #315 sample/instrument browser + waveform
  - **P3 larger feature:** #313 WAV/stem export + MediaRecorder capture · #322 lightweight pattern editor mode
- Items below are **runtime-verification debt** (code landed + typecheck/build green, but never confirmed in a live WebGPU browser) plus UI nits — none are open GitHub issues.
- [ ] **VERIFY v0.52/0.53/0.54 Night trio (runtime):** landed + merged 07-10, build/tests green. Pending browser smoke-test — all three appear in the shader picker and switch cleanly, playhead arc visible on each, per-instrument palette mode works when enabled, no `[WebGPU]` pipeline/WGSL parse errors (`.swarm-state.md` Night-trio section).
...
- **THIS RUN'S decoupled Copilot candidate → visual-accessibility slice of #317** (reduced-motion + color-blind-safe palettes + high-contrast theme). Touches `utils/shaderRegistry.ts` (motion/intensity uniform flags), a new `utils/accessiblePalettes.ts`, a small settings-toggle component under `components/`, and CSS — **zero overlap** with the audio path (`public/worklets/`, `openmpt-worklet.js`, `audio-worklet/`, `hooks/useAudioGraph.ts`/`useLibOpenMPT.ts`/`useWorkletLoader.ts`, `play()` call sites) that the Fix-First kimi-cli run touches. Deliberately excludes the ARIA-transport / screen-reader-announcement part of #317 (that reads transport state near the audio hooks) to stay decoupled. (#312 Vitest already landed; prior ChannelMeters VU fallback still available.)
...
## Done
- [x] 2026-08-07 — AUDIO-WORKLET LIFECYCLE LOCK LANDED (last week's Fix First focus): Copilot **#362** ("test(audio): lock #354 worklet lifecycle regressions with behavioral guards", merged 07-31) + **#364** (typed validated AudioWorklet message protocol boundary, merged 07-31) shipped the regression guards for the #354 modes (position-flood throttle, node-reuse on reload, stale-`loaded`-ack rejection). **Re-flagged Fix First anyway** — the guards are unit-level (helpers + source-string invariants) and did NOT catch the week's new audio breakage (see below). #363 (last run's decoupled Copilot unit — structural coverage assertions for visual smoke, catches thin bands / scanline offsets / overlay misalignment) also LANDED + merged 07-31.
- [x] 2026-08-07 — AUDIO REGRESSED A 4TH CYCLE (→ today's Fix First): **#375** (Claude, "cut per-quantum worklet cost + `?audioDiag=1` timing mode", merged 08-02) and **#376** (cursor, "XM pattern-boundary stutter — worklet quantum budget", merged 08-02, `WORKLET_VERSION` 7→9, protocol constants v=2) fixed pattern-boundary hitches from per-quantum WASM cost (get_time_at_position + per-channel VU + SAB updates now gated behind the 60 Hz path; projectm-pcm now opt-in; interpolation filter 8→4). Plus **#372** (cursor, merged 08-01) fixed volume/pan sliders not reaching the master output chain (analyser→panner→gain→destination re-asserted on every play/reload/SP-fallback; `getMasterGainValue`/`getMasterPanValue` test hooks added). All three shipped green under the #362 unit guards → confirms the guard gap that #380 closes.
- [x] 2026-08-07 — v0.30b PUBLIC SHIP (major product pivot, not from Ideas): #366/#367/#368/#369/#373 locked `patternv0.30b.wgsl` as the public default and shipped a minimal public surface — `VITE_PUBLIC_MODE=1` baked into `build:xm-player`/`build:public`, 3D studio / media overlay / export / MIDI / edit-mode / share / engine-toggle / reactive / debug all gated behind `IS_PUBLIC_MODE`, Controls `minimalSurface`, package version→0.1.0, SW cache `mod-player-v5`, bundle-budget asset-path fix for `/xm-player/` base. #365 (code-split 3D Studio + serve COOP/COEP on preview/CI) supports it. Live at `test.1ink.us/xm-player/`.
- [x] 2026-08-07 — LOCKFILE + DEP CHURN (watch): package-lock.json deleted (`7931a66`, 08-04) then restored/regenerated for vitest 4 / esbuild 0.28.1 nested peer tree (#374 08-02, #378 08-04, plus `.cursor/environment.json` for Cloud `npm ci`). Jules **#377** cached WebGL2Bloom uniform locations (perf). NOTE: 8 direct-to-main `push fix` commits by root 08-02→08-04 (unlabeled) — instability signal; foundation asserted from merge history, local build NOT run (fresh container).
- [x] 2026-07-31 — SHADER BLANK-RENDER CASCADE LANDED + CLOSED (prior Fix First focus): all six live-QA render bugs closed 2026-07-26 — black cluster #346 (Night trio v0.52/53/54), #347 (v0.56 Instrument Palette), #348 (v0.55 Oscilloscope) + partial cluster #349 (v0.24 Tunnel), #350 (v0.23 Clouds), #351 (WebGL2 overlay). Root causes fixed per `.swarm-state.md` (osc texture co-located with bind-group build in `useWebGPURender.ts`, racy create/destroy effect removed from `PatternDisplay.tsx`, `instrumentPalette.ts` empty-palette now emits full hue ramp, night-theme BG/LED_OFF floors raised, idle midIntensity 0.02→0.06); luminance-based black-frame guard added to `scripts/visual-smoke.mjs` + v0.52–v0.57 added to CI shader set. WGSL duplicate-declaration compile-error follow-up merged as **#353** (`29b5b6f`, 07-29). CI `visual-smoke` job runs `smoke:visual:ci` + WebGPU night-trio `smoke:visual`.
- [x] 2026-07-31 — STRATEGIC + FEATURE ISSUE SWEEP: the strategic set #335–#344 (filed 07-21) and remaining feature issues (#313/#315/#317/#321/#322/#332) all CLOSED 2026-07-26. Combined with the render-bug closes, **the tracker is now empty (0 open issues, 0 open PRs).** No unfinished user Idea remains → future non-Fix-First runs are New Idea mode.
- [x] 2026-07-31 — AUDIO REGRESSION #354 PATCHED (but re-flagged Fix First): `21cb5b6` (merged 07-29) fixed MOD hiccups (throttle worklet position postMessage to ~60 Hz, was ~350 Hz) + XM module-reload silence (skip node disconnect/reconnect via `canReuseWorkletNode`, module load token + `forceModuleLoad`, ignore stale `loaded` acks on token mismatch), `WORKLET_VERSION`→7. Touched `useAudioGraph.ts`/`useLibOpenMPT.ts`/`useWorkletLoader.ts`/`openmpt-worklet.js` + `tests/workletAudioLifecycle.test.ts`. **Third worklet-audio regression in three weeks with no failing-first guard → this run's Fix First focus (see Today's focus).**
- [x] 2026-07-31 — DEPLOY STALE-INDEX #355 FIXED: `f77dd6a` (merged 07-29) — `deploy.py` detects a stale `/xm-player/` directory index that served an old bundle and broke XM audio. Third deploy/base-path fix (#326/#327/#355) → carried as Claude Code hygiene re-exercise.
- [x] 2026-07-24 — AUDIO FIX-FIRST FULLY LANDED (prior focus, now closed): PR **#330** AudioContext keep-alive merged (`539f0a3`) — no `suspend()` on normal stop/reload, duplicate-`play()` guard, native-engine probe requires `openmpt-native.wasm`, worklet `?v=6` + SW `mod-player-v4` cache-bust; **#334** fractional `noteAge` worklet-sync follow-up merged (`5fbd056`); **#333** offline WAV export + channel mute mask + perf capture merged. Combined with #329 (merged 07-17) the silent-playback cascade is cured across every entry path. **0 open PRs.** Remaining regression-guard debt reformulated as open issue #336 (→ this run's Copilot unit).
- [x] 2026-07-24 — RECONCILE: strategic issue set **#335–#344** filed 2026-07-21 (P0 CI-Vitest-gate #335, P0 audio-regression-harness #336, P1 MainLayout store #337 / split useWebGPURender #338 / worklet A/V sync #339 / finish WGSL include migration #340, P2 a11y #341 / instrument inspector #342 / native-engine platform #343 / pattern-editor completion #344); Noah's live-QA render bugs **#346–#351** filed 2026-07-22. Both reconciled into Backlog by cluster/priority. Tracker now has 16 open issues (0 open PRs).
- [x] 2026-07-17 — #318 WGSL MODULAR INCLUDE SYSTEM (last week's User-Idea focus) LANDED + merged: `scripts/sync-shaders.mjs` resolves `//#include` offline into flat `public/shaders/`; `shaders/lib/` fragment library (`notes`/`pitch`/`dura`/`palette`/`sdf`/`tonemap`/`color_preserve`/`top_emitter`/`lens_cap` + `theme_night_52/53/54`); Night trio v0.52/0.53/0.54 fully migrated (byte-identical or comment-only diffs vs baseline), v0.51 partially migrated as a cross-family proof point; `tests/shaderIncludes.test.ts` (5/5); tsc build excludes Node-only tests (#324). All octave/trigger/duration/registry parity tests green (`.swarm-state.md` #318 log).
- [x] 2026-07-17 — #312 VITEST HARNESS (last week's decoupled Copilot unit) LANDED: `npm test` runs 50 Vitest tests; packing/parity scripts ported to real unit tests. Green across #324/#325/#329.
- [x] 2026-07-17 — AUDIO REGRESSION #329 MERGED: worklet-scope libopenmpt singleton — init once per `AudioWorkletGlobalScope`, reuse the AudioWorkletNode on module reload (hot `load` only), keep node alive across `stopMusic(false)`, `create_from_memory2` + tight byte slices, `WORKLET_VERSION`→5 cache-bust. Cured silent XM-after-module-switch + runaway `ended→seek 0` loop. (Second half — AudioContext keep-alive — still open in draft #330 → Fix-First focus.)
- [x] 2026-07-16 — DEPLOY/BASE-PATH HARDENING: #326 default prod build back to root base `/` (fixes 404 asset paths at site root / Cursor Cloud / `npm run preview`), `build:xm-player` still bakes `/xm-player/`; #327 `deploy.py` runs `build:xm-player:verify` before upload by default (`--no-build` to skip), `scripts/verify-build.mjs` rejects root-base index when `EXPECTED_BASE=/xm-player/`, adds `npm run deploy` / `deploy:upload-only`. #325 Cursor Cloud dev-env setup + `npm ci` verified (50/50 tests, build green).
- [x] 2026-07-10 — NIGHT TRIO v0.52/0.53/0.54 (prior User Idea focus): `patternv0.52.wgsl` (Night), `patternv0.53.wgsl` (Midnight), `patternv0.54.wgsl` (Neon Night) — dark-theme circular variants, uniform struct matching v0.56 (132-byte, paletteMode slot 32, instrumentPalette @binding(7)), v0.51 playhead-arc overlay copied verbatim (ARC-001 + bottom-12% UI-band discard), v0.56 per-instrument palette sampling path, three-emitter/DURA/ACES unchanged. Registered in shaderRegistry.ts + appConfig.ts. `sync:shaders`/`typecheck`/`build`/`test:shader-registry`/`test:octave-brightness`/`test:trigger-tail`/`test:duration-parity` all green (`.swarm-state.md`). Merged to main (branch even with main, 0 open PRs). Runtime browser verify pending → Backlog.
- [x] 2026-07-10 — Pipeline-hygiene pass merged (PR #294) + gitignore Python bytecode from deploy.py.
- [x] 2026-07-10 — Noah filed 14 feature issues #307–#322 (formalizing accumulated ideas into the tracker). Reconciled into Backlog by priority band.
- [x] 2026-07-03 — v0.30b "Note-On Disc" circular shader (spontaneous mid-week work, NOT from Ideas list): new `patternv0.30b.wgsl` keeping the v0.30 chrome-disc look but replacing channel-wide lane blinking with v0.45b playhead-scrolled sustain (DURA/TRIG-001). Iterated over PR #286 (create) → #288 (trigger-only persistent note-on) → #289 (steady glow, dim idle) → #290 (fix cyan LEDs always lit — `drawChromeIndicator` ignored `isOn`) → #291 (final: cyan holds until note-off, instant note-on, end fade). All merged to main; registered in appConfig/shaderRegistry/shaderVersion + `usesPlayheadRowAsFloat()` in useWebGPURender.
- [x] 2026-07-03 — VOLUME/VELOCITY-REACTIVE LEDs (last week's New Idea focus, VEL-001): `patternv0.57.wgsl` (copied from v0.50) — bottom emitter repurposed to a cyan velocity/volume indicator for pitched notes, top/mid emitter intensity scaled by `normalizedCellVolume()` (decoded from existing packed bytes, no packing change), organic sin-pulse on sustain tails; `octaveBrightness` + expression/note-off/empty paths untouched. Registered in shaderRegistry/appConfig/shaderVersion, wired through PatternDisplay + useWebGPURender chains. typecheck + build + octave-brightness/trigger-tail/duration-parity/shader-registry tests all green (`.swarm-state.md` VEL-001). Landed in code + merged to main (runtime browser verify pending → Backlog).
- [x] 2026-06-27 — Build footgun fixed (#287): `vite.config.ts` now uses `loadEnv(mode, cwd, 'VITE_')` so `.env.production`'s `VITE_APP_BASE_PATH=/xm-player/` actually applies to a plain `npm run build` (Vite doesn't populate `process.env` from `.env` files). Was a silent no-op previously caught only by `deploy.py`'s validate backstop.
- [x] 2026-07-03 — RECONCILE: all remaining open GitHub issues (#252, #269, #270, #272, #280) and the #278 draft PR closed/merged. Tracker is now empty (0 open issues, 0 open PRs). Runtime-verification debt for these carried into Backlog as browser-smoke-test items (not open issues).
- [x] 2026-06-26 — PRODUCTION LOAD PIPELINE (#273–#277) CLOSED + DEPLOYED (Fix First focus, 2026-06-19): the `c1e3854` fix push was verified, and the final subpath-deploy gaps fixed via PR #281 (`utils/storageApi.ts` production fallback to `https://storage.noahcohn.com`; `build:xm-player` bakes `VITE_STORAGE_API_URL`; `useLibOpenMPT.ts` prefers main-thread parse when libopenmpt is ready, skipping the slow CDN-refetching parser worker under strict COEP; `audio-worklet/diagnostics.ts` logs `detectRuntimeBase()`). Issue-draft docs merged PR #278. **All five issues #273–#277 now CLOSED**; live app at `test.1ink.us/xm-player/` loads → `Fetching` → `Parsing` → `Loaded`, no `/api` 404s, stylesheet `.css` not `.1iss`, COEP `credentialless`.
...
## Last run
Date: 2026-08-07
Mode: **Fix First** (audio-lifecycle lock #362 landed but audio regressed a 4th cycle #375/#376 + slider bug #372 the same week, all green under the unit guards; P0 #380 filed to close the guard gap; app just went public)
Focus: Implement #380 — real-browser Playwright audio product-flow smoke against `npm run preview` `?engine=js` (play→MOD↔XM→stop→play, assert counters advance, fail on worklet/protocol console errors + `window.__AUDIO_DIAG__` pattern-boundary quantum overruns), wired into CI beside `visual-smoke`. Test/CI/tooling files only — no audio-hook, worklet, or shader edits.
Outcome: Last week's audio-lifecycle lock LANDED — Copilot #362 (behavioral guards for the #354 modes) + #364 (typed worklet protocol boundary) merged 07-31, and the decoupled Copilot visual unit #363 (structural coverage assertions) merged 07-31. **But the lock proved unit-level and insufficient:** audio regressed a 4th time within days — #375/#376 (XM pattern-boundary stutter from per-quantum worklet CPU overshoot, `WORKLET_VERSION` 7→9, merged 08-02) and #372 (master gain/pan chain broken) — all shipping green under the new guards. Meanwhile Noah pivoted hard to **public**: v0.30b locked as the public default with a minimal surface (#366–#369/#373, `VITE_PUBLIC_MODE=1`, version→0.1.0, SW `mod-player-v5`). A freshly-public player whose CI can ship silent/stuttering audio is a cracked foundation → **Fix First** on #380 (P0, filed 08-05: "CI can ship green while speakers are silent"), which takes priority over the six open refactor/hygiene issues (#370/#371/#381/#382/#383/#384). Decoupled Copilot unit = **accessibility visual slice of #317** (reduced-motion + color-blind-safe palette + high-contrast toggle — render/UI/CSS only, zero overlap with kimi's test/CI path; newly timely now the app is public). Claude Code whole-stack = validate the new public build + `npm ci` on the restored lockfile → `build:xm-player` → `verify:build` → `deploy.py` dry-run. NOTE: chat-history tools (recent_chats/conversation_search) unavailable again this run — reconciliation done from git log + GitHub issues/PRs. Local build/test NOT run (fresh container); foundation state asserted from merge history (7 open issues; #362/#364/#372/#375/#376/#377/#378 merged 07-31→08-04) + 8 unlabeled `push fix` direct-to-main commits 08-02→08-04.
