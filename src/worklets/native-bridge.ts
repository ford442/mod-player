/**
 * native-bridge.ts — TypeScript source of record.
 *
 * Built to public/worklets/native-bridge-processor.js by
 * scripts/build-worklet-js.mjs. **Edit this file, never the generated output.**
 *
 ...
 * must match RING_HEADER in cpp/worklet_processor.cpp:
...
/** processorOptions passed from the main thread via new AudioWorkletNode(...). */
interface NativeBridgeOptions {
  /** The WASM linear memory buffer (shared). */
  wasmMemory?: unknown;
  /** Byte offset of the ring buffer inside wasmMemory. */
  ringBufByteOffset?: unknown;
  /** Ring buffer size in stereo frames. */
  frameCapacity?: unknown;
}

class NativeBridgeProcessor extends AudioWorkletProcessor {
    _active!: boolean;
    _starvationCount!: number;
    _lastStarvationLog!: number;
    _frameCount?: number;
    _writeHeadView?: Int32Array;
    _readHeadView?: Int32Array;
    _samplesView?: Float32Array;
    _frameCapacity?: number;

    constructor(options?: AudioWorkletNodeOptions) {
        super(options);

        const { wasmMemory, ringBufByteOffset, frameCapacity }: NativeBridgeOptions =
            (options && options.processorOptions)
                ? options.processorOptions as NativeBridgeOptions
                : {};
