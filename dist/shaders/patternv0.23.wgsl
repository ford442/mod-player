...
// ── Vertex Shader (Fits nicely inside chassis window) ──
@vertex
fn vs(@builtin(vertex_index) idx: u32) -> VertexOut {
  ...
  // Fit to chassis display window
  let scale = vec2<f32>(0.91, 0.74);
  let offset = vec2<f32>(0.0, 0.2);

  out.position = vec4<f32>(rawPos * scale + offset, 0.0, 1.0);
...
fn fs(fragIn: VertexOut) -> @location(0) vec4<f32> {
  ...
  var finalColor = vec3<f32>(0.05, 0.05, 0.08); // Dark stage
  ...
  let uvMask = step(0.0, charUV.x) * step(charUV.x, 1.0) * 
               step(0.0, charUV.y) * step(charUV.y, 1.0);
