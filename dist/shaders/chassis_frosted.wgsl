...
fn fs(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    if (sdRoundedBox(p - volPos, volDim, 0.003) < 0.0) { 
    ...
    if (dVolHandle < 0.0) { 
        color = mix(color, vec3<f32>(0.3, 0.9, 0.5), smoothstep(aa, -aa, dVolHandle)); 
    }

    // Pan slider
    let sliderRightX = 0.42;
    let sliderY = -0.2;
    if (sdRoundedBox(p - vec2<f32>(sliderRightX, sliderY), vec2<f32>(0.008, 0.1), 0.003) < 0.0) { 
        color = vec3<f32>(0.2, 0.2, 0.2); 
    }
    ...
    if (dPanHandle < 0.0) { 
        color = mix(color, vec3<f32>(0.4, 0.6, 1.0), smoothstep(aa, -aa, dPanHandle)); 
    }
    ...
    if (dBPM < 0.0) { 
        color = mix(color, lcdColor, smoothstep(aa, 0.0, dBPM)); 
    }
    ...
    if (dRow < 0.0) { 
        color = mix(color, lcdAmber, smoothstep(aa, 0.0, dRow)); 
    }
    let dOrd = drawNumber(p - vec2<f32>(-0.10, posY), u32(bez.currentOrder), 2u, 0.01, 0.012);
    if (dOrd < 0.0) { 
        color = mix(color, lcdAmber, smoothstep(aa, 0.0, dOrd)); 
    }
