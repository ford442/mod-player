// patternv0.59.wgsl
// Instrument-highlight variant of v0.56.
// When highlightInstrument > 0, matching mid LEDs are boosted and others dimmed.
// paletteMode behavior matches v0.56.
...
struct Uniforms {
  ...
  _r3: f32,
  colorPalette: u32,
  // Padding that mirrors the TS uniform payload layout so paletteMode lands at slot 32.
  innerRadius: f32,
  outerRadius: f32,
  ...
  invertMix: f32,
  paletteMode: u32,
  highlightInstrument: u32, // slot 33 — 0=off, else 1-based instrument index
};
...
fn fs(in: VertexOut) -> @location(0) vec4<f32> {
  ...
  if (inButton > 0.5) {
    ...
    if (uniforms.highlightInstrument > 0u && (isNoteOn || isSustain)) {
