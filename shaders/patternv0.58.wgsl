// patternv0.58.wgsl
// Three-Emitter LED + multi-band audio-reactive chassis (REACT-001)
// Based on v0.57 (velocity LEDs) with shared AudioReactive uniform at binding 8.
// DURA + VEL-001 retained; kick/bloom driven from worklet SAB bands when enabled.
...
// VEL-001: Decode per-step volume/velocity from the frozen packed cell layout.
// packedA bits [7:0] hold a compressed volCmd/volVal nibble; packedB bits [7:0]
// hold the full volCmd byte. "No volume column" (both zero) is treated as full
// velocity, because modules omit the volume column constantly.
fn normalizedCellVolume(packedA: u32, packedB: u32) -> f32 {
  let volPacked = packedA & 0xFFu;
  let coarseVol = volPacked & 0x0Fu;        // 0..15 reconstruction of volVal
  let fullVolCmd = packedB & 0xFFu;         // original volume command byte
...
fn drawUnifiedLensCap(
    ...
    let botPos = vec2<f32>(0.0, 0.28);     // Bottom: Velocity/volume indicator
...
fn fs(in: VertexOut) -> @location(0) vec4<f32> {
  ...
  if (inButton > 0.5) {
    ...
    // VEL-001: per-step volume/velocity factor for pitched notes only
    ...
    // VEL-001: louder pitched notes glow brighter on the top semantic emitter
    ...
    // EMITTER 3 (BOTTOM): Velocity/volume indicator — only for pitched notes
    var botIntensity = 0.0;
    var botColor = vec3<f32>(0.0);
    let velocityColor = vec3<f32>(0.05, 0.85, 1.0); // cyan
    if (!isMuted && (isNoteOn || isSustain)) {
      if (isNoteOn) {
    ...
    let unifiedLens = drawUnifiedLensCap(
        ...
        vec4<f32>(botColor, botIntensity),    // Bottom: Velocity/volume
        aa
    );
  ...
  // Kick reactive glow — SAB bands when reactive mode on, else legacy kickTrigger
