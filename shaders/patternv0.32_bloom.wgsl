// patternv0.32_bloom.wgsl
// - Brighter Blinks on Trigger
// - Slower Decay
// - Removed Playhead Background Highlight
...
fn fs(in: VertexOut) -> @location(0) vec4<f32> {
  ...
  if (inButton > 0.5) {
    ...
    // Slow decay logic for the LED
    let decayActivity = exp(-ch.noteAge * 1.5);
    let isActive = (decayActivity > 0.05) && !isMuted;

    let topColorBase = vec3(0.0, 0.9, 1.0);
    let topColor = topColorBase * (bloom * 8.0) * decayActivity;
    ...
    if (hasNote) {
      ...
      // Flash logic: Set to 1.0 for max brightness on trigger
      let flash = f32(ch.trigger) * 1.0;

      // Calculate spatial distance from playhead (optional trail)
      var d = f32(in.row) + uniforms.tickOffset - f32(uniforms.playheadRow);
      let totalSteps = 64.0;
      if (d > totalSteps * 0.5) { d = d - totalSteps; }
      if (d < -totalSteps * 0.5) { d = d + totalSteps; }
      let coreDist = abs(d);
      let energy = 0.02 / (coreDist + 0.001); // Distance glow
      let trail = exp(-10.0 * max(0.0, -d));  // Short trail

      let activeVal = clamp(pow(energy, 1.5) + trail, 0.0, 1.0);

      // Combine: Spatial glow + Immediate Flash + Slow Linger
      lightAmount = (activeVal * 0.8 + flash + (linger * 2.0)) * clamp(ch.volume, 0.0, 1.2);
      if (isMuted) { lightAmount *= 0.2; }
    }

    // HDR: Boost colors significantly if lightAmount is high
    ...
    // REMOVED: Component 4 (Playhead Glance)
  }
