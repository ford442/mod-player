// patternv0.33_bloom.wgsl
// - Blue LEDs now indicate "Expression Data" on the specific step (not channel activity)
// - Notes flash brighter when playhead hits them
...
fn vs(@builtin(vertex_index) vertexIndex: u32, @builtin(instance_index) instanceIndex: u32) -> VertexOut {
  ...
  // --- LAYOUT FLIP: 0 is Outer ---
  let invertedChannel = numChannels - 1u - channel;
  // Honor invert flag (1 = inner low, 0 = outer low)
...
fn fs(in: VertexOut) -> @location(0) vec4<f32> {
  ...
  if (inButton > 0.5) {
    ...
    // Decode packedB to check for effects/volume
    ...
    // Check if this specific cell has "expression data" (Vol/Pan or Effect)
    ...
    // FIX: Only illuminate if THIS STEP has expression data.
    ...
    // Static glow if data is present, no flashing needed for this indicator
    ...
    if (isDataPresent) {
         finalColor += topColor * topLed.a * 0.3;
    }
    ...
    if (hasNote) {
      ...
      // Slow decay (linger) from channel state
      let linger = exp(-ch.noteAge * 1.5);

      // FIX: Add "Playhead Strike" intensity
      // If this row is the playhead row, flash BRIGHT!
      let onPlayhead = (in.row == uniforms.playheadRow);
      let strike = select(0.0, 3.0, onPlayhead); // Instant burst

      // Also allow trigger from audio engine (flash)
      let flash = f32(ch.trigger) * 1.0;

      // Distance glow (spatial)
      ...
      // Combine: Linger + Strike + Flash + Spatial
      // Strike (3.0) and Flash (1.0) push it well into HDR territory
    ...
    // This pill specifically visualizes the Effect Code color
    if (effCode > 0u) {
