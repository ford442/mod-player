// patternv0.34_bloom.wgsl
// - "Night Mode": Dims hardware when playing
// - UV Purple Ring: Outer ring turns violet and glows onto bezel
// - Expression Blue: Blue LEDs only for data steps
...
fn drawChromeIndicator(
    ...
    isOn: bool,
    aa: f32,
    dimFactor: f32  // NEW: controls how dark the chrome is
) -> vec4<f32> {
...
fn fs(in: VertexOut) -> @location(0) vec4<f32> {
  ...
  if (in.channel == 0u) {
    ...
    let uvPurple = vec3(0.65, 0.0, 1.0); // Deep Violet
    let activePurple = vec3(0.8, 0.4, 1.0); // Bright Violet

    var indColor = standardGray;
    if (isPlaying) {
        indColor = select(uvPurple, activePurple, onPlayhead);
    } else if (onPlayhead) {
        indColor = fs.ledOnColor; // Standard Cyan if not playing
    }

    // Always Lit if playing (for that UV ring look)
  ...
  var finalColor = fs.bgColor * dimFactor; // Apply dimming to housing background
  ...
  if (inButton > 0.5) {
    ...
    if (hasNote) {
      ...
      // PLAYHEAD STRIKE: Instant flash when playhead hits
      let onPlayhead = (in.row == uniforms.playheadRow);
      ...
      // compute activeVal (distance-based glow)
