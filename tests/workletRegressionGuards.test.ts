...
describe('#354 production source invariants', () => {
  ...
  it('WORKLET_VERSION stays cache-busted at ≥ 17 after the generated-worklet switch', () => {
    const m = useWorkletLoader.match(/WORKLET_VERSION\s*=\s*['"](\d+)['"]/);
    expect(m, 'WORKLET_VERSION must be defined').toBeTruthy();
    expect(Number(m![1])).toBeGreaterThanOrEqual(17);
  });

  it('the production worklet is generated, never hand-edited', () => {
    // These invariants are checked against the esbuild output; the file they
    // describe is compiled from src/worklets/openmpt-processor.ts.
    expect(workletSource).toContain('@generated by scripts/build-worklet-js.mjs');
    expect(workletSource).toContain('src/worklets/openmpt-processor.ts');
  });
